...
from pyshape.utils.geometry import get_all_operators, to_basis, from_basis, compute_hks_autoscale, \
...
class LearnedTimeDiffusion(nn.Module):
    ...
    def forward(self, feat, L, mass, evals, evecs):
        ...
        assert feat.shape[
                   -1] == self.in_channels, f'Expected feature channel: {self.in_channels}, but got: {feat.shape[-1]}'
...
class DiffusionNetBlock(nn.Module):
    ...
    def forward(self, feat_in, mass, L, evals, evecs, gradX, gradY):
        ...
        B = feat_in.shape[0]
        assert feat_in.shape[
                   -1] == self.in_channels, f'Expected feature channel: {self.in_channels}, but got: {feat_in.shape[-1]}'
