...
class RegularizedFMNet(nn.Module):
    ...
    def __init__(self, lmbda=100, resolvant_gamma=0.5, bidirectional=True):
    ...
    def forward(self, feat_x, feat_y, evals_x, evals_y, evecs_trans_x, evecs_trans_y):
        ...
        for i in range(evals_x.shape[1]):
            ...
            try:
                ...
                warnings.warn(f"The matrix of size {(A_A_t + self.lmbda * D_i).size()} is not invertible!")
                C = torch.bmm(torch.pinverse(A_A_t + self.lmbda * D_i), B_A_t[:, [i], :].transpose(1, 2))
        ...
        if self.bidirectional:
            ...
            for i in range(evals_y.shape[1]):
                ...
                try:
                    ...
                    warnings.warn(f"The matrix of size {(B_B_t + self.lmbda * D_i).size()} is not invertible!")
                    C = torch.bmm(torch.pinverse(B_B_t + self.lmbda * D_i), A_B_t[:, [i], :].transpose(1, 2))
                C_i.append(C.transpose(1, 2))
